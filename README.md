...
- **备注** — 学习过程中随时记录当前学的内容（暂停时也可编辑），历史记录中可修改/补充/清空已保存的备注
...
| PATCH  | `/api/records/:id`             | 修改学习记录的备注         |
...
```text
client/package.json → 0.2.4   (客户端版本，独立递增)
server/package.json → 0.2.4   (服务端版本，独立递增)
git tag              → v0.2.7 (项目里程碑标记)
```
